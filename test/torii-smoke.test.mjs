...
// Skipped when the torii on PATH is not the pinned version (asdf install torii <version>).
...
import { generate, loadContracts, pinnedToriiVersion, ROOT } from './helpers.mjs'

const pinned = pinnedToriiVersion()
// cwd matters: the asdf shim reads .tool-versions from the working directory.
const local = spawnSync('torii', ['--version'], { encoding: 'utf8', cwd: ROOT })
const localVersion = local.status === 0 ? /torii (\S+)/.exec(local.stdout)?.[1] : undefined
const skip = localVersion !== pinned ? `torii ${pinned} not on PATH (found ${localVersion ?? 'none'})` : false

for (const network of Object.keys(loadContracts())) {
  test(`${network}: torii ${pinned} boots with the generated config`, { skip, timeout: 90_000 }, async () => {
    ...
    const child = spawn('torii', ['--config', configPath, '--relay.port', '0', '--relay.webrtc_port', '0', '--relay.websocket_port', '0', '--grpc.addr', '127.0.0.1', '--grpc.port', '0'], { cwd: ROOT })
    let log = ''
